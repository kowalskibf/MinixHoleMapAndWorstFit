...
phys_clicks clicks; /* amount of memory requested */
{
    ...
    register struct hole *hp, *prev_ptr, *max_h, *p_max_h;
    ...
    else
    {
        do
        {
            ...
            if (hp->h_len >= clicks)
            {
                /* We found a hole that is the biggest one.  Use it. */
                old_base = max_h->h_base; /* remember where it started */
                max_h->h_base += clicks;  /* bite a piece off */
                max_h->h_len -= clicks;   /* ditto */

                /* Delete the hole if used up completely. */
                if (max_h->h_len == 0)
                    del_slot(p_max_h, max_h);
...
int do_hole_map()
{
    ...
    unsigned int pairsReal = nbytes / 2;
    unsigned int pairsExp = pairsReal;
    ...
    while (hp != NIL_HOLE && pairsReal > 0 && hp->h_base < swap_base)
    {
        buffer[i++] = hp->h_len;
        buffer[i++] = hp->h_base;
        hp = hp->h_next;
 ...
 *				del_slot				     *
 *===========================================================================*/
PRIVATE void del_slot(prev_ptr, hp) register struct hole *prev_ptr; /* pointer to hole entry just ahead of 'hp' */
...
PRIVATE int swap_out()
{
    ...
    do
    {
        ...
        if (rmp->mp_flags & (TRACED | REPLY | ONSWAP))
            continue;
